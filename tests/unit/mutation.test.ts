...
describe('Immutable Variants', () => {
  test('pushed() should return a new array with items appended', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3);

    const newArr = arr.pushed(4, 5);

    expect(arr.length).toBe(3);
    expect(arr.toArray()).toEqual([1, 2, 3]);
    expect(newArr.length).toBe(5);
    expect(newArr.toArray()).toEqual([1, 2, 3, 4, 5]);
  });

  test('pushed() should handle ArrayLike objects', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2);

    const newArr = arr.pushed([3, 4], new Uint8Array([5, 6]));

    expect(arr.length).toBe(2);
    expect(newArr.length).toBe(6);
    expect(newArr.toArray()).toEqual([1, 2, 3, 4, 5, 6]);
  });

  test('shifted() should return a new array with first element removed', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3);

    const newArr = arr.shifted();

    expect(arr.length).toBe(3);
    expect(arr.toArray()).toEqual([1, 2, 3]);
    expect(newArr.length).toBe(2);
    expect(newArr.toArray()).toEqual([2, 3]);
  });

  test('shifted() on empty array should return empty copy', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
  ...
  test('spliced() should return a new array with elements removed', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3, 4, 5);

    const newArr = arr.spliced(1, 2);

    expect(arr.length).toBe(5);
    expect(arr.toArray()).toEqual([1, 2, 3, 4, 5]);
    expect(newArr.length).toBe(3);
    expect(newArr.toArray()).toEqual([1, 4, 5]);
  });

  test('spliced() should return a new array with elements inserted', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 4);

    const newArr = arr.spliced(1, 0, 2, 3);

    expect(arr.length).toBe(2);
    expect(arr.toArray()).toEqual([1, 4]);
    expect(newArr.length).toBe(4);
    expect(newArr.toArray()).toEqual([1, 2, 3, 4]);
  });

  test('spliced() should handle simultaneous deletion and insertion', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 5);

    const newArr = arr.spliced(1, 1, 3, 4);

    expect(arr.length).toBe(3);
    expect(arr.toArray()).toEqual([1, 2, 5]);
    expect(newArr.length).toBe(4);
    expect(newArr.toArray()).toEqual([1, 3, 4, 5]);
  });

  test('unshifted() should return a new array with items prepended', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(3, 4, 5);

    const newArr = arr.unshifted(1, 2);
  ...
  test('truncated() should return a new array with reduced length', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3, 4, 5);

    const newArr = arr.truncated(3);

    expect(arr.length).toBe(5);
    expect(arr.toArray()).toEqual([1, 2, 3, 4, 5]);
    expect(newArr.length).toBe(3);
    expect(newArr.toArray()).toEqual([1, 2, 3]);
  });

  test('filled() should return a new array with filled values', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3, 4, 5);

    const newArr = arr.filled(0);

    expect(arr.length).toBe(5);
    expect(arr.toArray()).toEqual([1, 2, 3, 4, 5]);
    expect(newArr.length).toBe(5);
    expect(newArr.toArray()).toEqual([0, 0, 0, 0, 0]);
  });

  test('filled() should respect start and end parameters', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3, 4, 5);

    const newArr = arr.filled(0, 1, 3);

    expect(newArr.toArray()).toEqual([1, 0, 0, 4, 5]);
  });

  test('cleared() should return a new empty array', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3);

    const newArr = arr.cleared();
  ...
  test('reversed() should return a new array with reversed elements', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(1, 2, 3);

    const newArr = arr.reversed();

    expect(arr.length).toBe(3);
    expect(arr.toArray()).toEqual([1, 2, 3]);
    expect(newArr.length).toBe(3);
    expect(newArr.toArray()).toEqual([3, 2, 1]);
  });

  test('reversed() on empty array should return empty copy', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
  ...
  test('sorted() should return a new array with sorted elements', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(30, 10, 20);

    const newArr = arr.sorted();

    expect(arr.length).toBe(3);
    expect(arr.toArray()).toEqual([30, 10, 20]);
    expect(newArr.length).toBe(3);
    expect(newArr.toArray()).toEqual([10, 20, 30]);
  });

  test('sortedWith() should return a new array with custom sorted elements', () => {
    const arr = new DynamicArray<Uint8ArrayConstructor>();
    arr.push(30, 10, 20);

    const newArr = arr.sortedWith((a, b) => b - a);

    expect(arr.length).toBe(3);
    expect(arr.toArray()).toEqual([30, 10, 20]);
    expect(newArr.length).toBe(3);
    expect(newArr.toArray()).toEqual([30, 20, 10]);
  });

  test('immutable methods should preserve TypedArray constructor', () => {
    const arr = new DynamicArray<Float64ArrayConstructor>(
      10,
      Infinity,
      Float64Array,
    );
    arr.push(3.14, 1.0, 2.5);

    const pushed = arr.pushed(4.0);
    const shifted = arr.shifted();
    const spliced = arr.spliced(1, 1, 9.9);
    const unshifted = arr.unshifted(0.0);
    const truncated = arr.truncated(2);
    const filled = arr.filled(0);
    const cleared = arr.cleared();
    const reversed = arr.reversed();
    const sorted = arr.sorted();
    const sortedWith = arr.sortedWith((a, b) => a - b);

    expect(pushed.getTypedArrayCtor()).toBe(Float64Array);
    expect(shifted.getTypedArrayCtor()).toBe(Float64Array);
    expect(spliced.getTypedArrayCtor()).toBe(Float64Array);
    expect(unshifted.getTypedArrayCtor()).toBe(Float64Array);
    expect(truncated.getTypedArrayCtor()).toBe(Float64Array);
    expect(filled.getTypedArrayCtor()).toBe(Float64Array);
    expect(cleared.getTypedArrayCtor()).toBe(Float64Array);
    expect(reversed.getTypedArrayCtor()).toBe(Float64Array);
    expect(sorted.getTypedArrayCtor()).toBe(Float64Array);
    expect(sortedWith.getTypedArrayCtor()).toBe(Float64Array);
  });
});
